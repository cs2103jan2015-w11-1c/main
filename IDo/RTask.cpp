...
void RTask::setFirstOccur(vector<string> parsedInfo){
	...
	if (parsedInfo.size() == 7 || parsedInfo.size() == 10) {	//deadline recurring task without limit
		_occur.setEndTime(_abstract.getEndTime());
		_occur.setEndDate(parsedInfo[2]);
	} else if(parsedInfo.size() == 9 || parsedInfo.size() == 12) { //timed task recurring without limit
